...
async function handleAntiBadwordCommand(sock, chatId, message, match) {
    if (!match) {
        return sock.sendMessage(chatId, {
            text: `*『 ANTIBADWORD SETUP 』*\n\n` +
                 `🔹 *!antibadword on*\n` +
                 `Turn on antibadword system\n\n` +
                 `🔹 *!antibadword set <action>*\n` +
                 `Set enforcement action: *delete* / *kick* / *warn*\n\n` +
                 `🔹 *!antibadword off*\n` +
                 `Disables antibadword in this group chat`
        }, { quoted: message });
    }
    ...
    if (cleanMatch === 'on') {
        ...
        if (existingConfig?.enabled) {
            return sock.sendMessage(chatId, { text: '⚠️ *AntiBadword is already enabled for this group.*' }, { quoted: message });
        }
        await setAntiBadword(chatId, 'on', 'delete');
        return sock.sendMessage(chatId, { text: '✅ *AntiBadword has been enabled.* Use *!antibadword set <action>* to customize moderation rules.' }, { quoted: message });
    }

    if (cleanMatch === 'off') {
        const config = await getAntiBadword(chatId, 'on');
        if (!config?.enabled) {
            return sock.sendMessage(chatId, { text: '⚠️ *AntiBadword is already disabled for this group.*' }, { quoted: message });
        }
        await removeAntiBadword(chatId);
        return sock.sendMessage(chatId, { text: '✅ *AntiBadword protection filter has been disabled for this group.*' }, { quoted: message });
    }

    if (cleanMatch.startsWith('set')) {
        const action = cleanMatch.split(' ')[1];
        if (!action || !['delete', 'kick', 'warn'].includes(action)) {
            return sock.sendMessage(chatId, { text: '❌ *Invalid action.* Choose from: *delete*, *kick*, or *warn*' }, { quoted: message });
        }
        await setAntiBadword(chatId, 'on', action);
        return sock.sendMessage(chatId, { text: `✅ *AntiBadword moderation enforcement action set to: ${action}*` }, { quoted: message });
    }

    return sock.sendMessage(chatId, { text: '❌ *Invalid arguments.* Use *!antibadword* to see correct configuration parameters.' }, { quoted: message });
}

async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    if (!chatId.endsWith('@g.us')) return; // Skip if not group
    if (message.key.fromMe) return; // Skip if message is from bot
    ...
    // Take action based on database action metrics
    switch (antiBadwordConfig.action) {
        case 'delete':
            await sock.sendMessage(chatId, {
                text: `⚠️ *🔒 Profanity Filter:* @${senderId.split('@')[0]}, using inappropriate language or swearing is strictly prohibited in this group!`,
                mentions: [senderId]
            });
            break;
        ...
        case 'kick':
            try {
                ...
                await sock.sendMessage(chatId, {
                    text: `🚨 *Automated Removal:* @${senderId.split('@')[0]} has been removed from the group for violating profanity policies.`,
                    mentions: [senderId]
                });
            } catch (error) {
        ...
        case 'warn':
            ...
            if (warningCount >= 3) {
                try {
                    ...
                    await sock.sendMessage(chatId, {
                        text: `🚨 *Automated Action:* @${senderId.split('@')[0]} has been removed after accumulating 3 automated warnings for foul language usage.`,
                        mentions: [senderId]
                    });
                } catch (error) {
                ...
                await sock.sendMessage(chatId, {
                    text: `⚠️ *Warning Alert:* @${senderId.split('@')[0]} you have received an automated warning (*${warningCount}/3*) for using bad words. Clean up your language!`,
                    mentions: [senderId]
                });
            }
            break;
