		...
		switch (action) {
			case 'delete':
				await sock.sendMessage(jid, { 
					text: `⚠️ *🔒 Antilink Filter:* @${sender.split('@')[0]}, sending links is not allowed in this group!`,
					mentions: [sender] 
				});
				break;
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `🚨 *Automated Removal:* @${sender.split('@')[0]} has been removed from the group for sending links.`,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `🚨 *Automated Action:* @${sender.split('@')[0]} has been removed after accumulating ${WARN_COUNT} warnings for link propagation.`,
						mentions: [sender]
					});
				} else {
					await sock.sendMessage(jid, {
						text: `⚠️ *Warning Alert:* @${sender.split('@')[0]} you have received an automated warning (*${warningCount}/${WARN_COUNT}*) for sending links. Rules compliance is mandatory!`,
						mentions: [sender]
					});
				}
				break;
