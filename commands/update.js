...
async function restartProcess(sock, chatId, message) {
    try {
        await sock.sendMessage(chatId, { text: '🔄 Update complete! Restarting system…' }, { quoted: message });
    } catch {}
    try {
        // Preferred: PM2
...
async function updateCommand(sock, chatId, message, zipOverride) {
    ...
    if (!message.key.fromMe && !isOwner) {
        await sock.sendMessage(chatId, { text: '❌ Only the bot owner or sudo users can use the !update command.' }, { quoted: message });
        return;
    }
    try {
        // Minimal UX
        await sock.sendMessage(chatId, { text: '🔄 Fetching updates from repository, please wait…' }, { quoted: message });
        if (await hasGitRepo()) {
        ...
        try {
            ...
            await sock.sendMessage(chatId, { text: `✅ Update core pulled successfully. Initializing restart…` }, { quoted: message });
        } catch {
            await sock.sendMessage(chatId, { text: '✅ Restored and updated structure successfully.\nType *!ping* to verify the server instance.' }, { quoted: message });
        }
        await restartProcess(sock, chatId, message);
    } catch (err) {
        console.error('Update failed:', err);
        await sock.sendMessage(chatId, { text: `❌ Update process failed:\n\n${String(err.message || err)}` }, { quoted: message });
    }
}
