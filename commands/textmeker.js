...
// Base channel info template branded for YOUR HUSBAND
const channelInfo = {
    forwardingScore: 1,
    isForwarded: true,
...
const messageTemplates = {
    ...
    success: (text, imageUrl) => ({
        image: { url: imageUrl },
        caption: "GENERATED BY YOUR HUSBAND ✨",
        contextInfo: channelInfo
    })
};
...
async function textmakerCommand(sock, chatId, message, q, type) {
    try {
        if (!q) {
            return await sock.sendMessage(chatId, messageTemplates.error(`⚠️ Please provide text to generate!\n\nExample: !${type || 'metallic'} Nick`));
        }
        ...
        try {
            ...
            switch (type) {
                ...
                case 'fire':
                    ...
                    break;
                default:
                    return await sock.sendMessage(chatId, messageTemplates.error("❌ Invalid text generator type configuration."));
            }
            ...
            await sock.sendMessage(chatId, messageTemplates.error(`❌ *YOUR HUSBAND ERROR* \n\nError: ${error.message}`));
        }
    } catch (error) {
