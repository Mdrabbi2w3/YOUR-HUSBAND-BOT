...
async function stickerCommand(sock, chatId, message) {
    try {
        ...
        if (!quotedMsg) {
            await sock.sendMessage(chatId, { text: '❌ Please reply to an image or video to make a sticker!' }, { quoted: message });
            return;
        }

        const type = Object.keys(quotedMsg)[0];
        if (!['imageMessage', 'videoMessage'].includes(type)) {
            await sock.sendMessage(chatId, { text: '❌ Please reply to an image or video!' }, { quoted: message });
            return;
        }
        ...
        // Send a text alert/notification before or with the sticker
        await sock.sendMessage(chatId, { 
            text: `🎉 *YOUR HUSBAND* \n\nHere is your sticker! Processing complete. ✨` 
        }, { quoted: message });

        // Send the actual sticker
        await sock.sendMessage(chatId, { 
