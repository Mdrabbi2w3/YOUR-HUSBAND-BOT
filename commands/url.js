...
async function urlCommand(sock, chatId, message) {
    try {
        ...
        if (!media) {
            await sock.sendMessage(chatId, { text: '⚠️ Please send or reply to a media file (image, video, audio, sticker, document) with the *!url* command.' }, { quoted: message });
            return;
        }
        ...
        if (!url) {
            await sock.sendMessage(chatId, { text: '❌ Failed to upload media to server.' }, { quoted: message });
            return;
        }

        await sock.sendMessage(chatId, { text: `🔗 *Generated URL:* ${url}` }, { quoted: message });
    } catch (error) {
