...
async function videoCommand(sock, chatId, message) {
    try {
        ...
        if (!searchQuery) {
            await sock.sendMessage(chatId, { text: '⚠️ What video do you want to download? Please provide a song title or link.' }, { quoted: message });
            return;
        }
        ...
        try {
            ...
            if (thumb) {
                await sock.sendMessage(chatId, {
                    image: { url: thumb },
                    caption: `🎥 *${captionTitle}*\n\n🔄 Downloading media file, please wait...`
                }, { quoted: message });
            }
        } catch (e) { 
        ...
        if (error.message && error.message.includes('blocked')) {
            errorMessage = '❌ Download blocked. The content may be unavailable in your region due to restrictions.';
        } else if (error.response?.status === 451 || error.status === 451) {
            errorMessage = '❌ Content unavailable (451) due to regional blocking or copyright restrictions.';
        } else if (error.message && error.message.includes('All download sources failed')) {
            errorMessage = '❌ All download sources failed. Please try again with a different video link.';
