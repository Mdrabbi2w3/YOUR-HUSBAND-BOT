...
async function sendAnimu(sock, chatId, message, type) {
    ...
    async function convertMediaToSticker(mediaBuffer, isAnimated) {
        ...
        // Add sticker metadata with YOUR HUSBAND BOT and RABBI
        const img = new webp.Image();
        await img.load(webpBuffer);

        const json = {
            'sticker-pack-id': crypto.randomBytes(32).toString('hex'),
            'sticker-pack-name': 'YOUR HUSBAND BOT STICKERS',
            'sticker-pack-publisher': 'RABBI',
            'emojis': ['🎌']
        };
    ...
    if (data.link) {
        ...
        try {
            await sock.sendMessage(
                chatId,
                { image: { url: link }, caption: `✨ *YOUR HUSBAND BOT* ✨\n\n*Anime:* ${type}\n*Powered by:* RABBI` },
                { quoted: message }
            );
            return;
        } catch {}
    ...
    await sock.sendMessage(
        chatId,
        { text: '❌ *YOUR HUSBAND BOT* ❌\n\nFailed to fetch anime data. Try again later.' },
        { quoted: message }
    );
}
    ...
    try {
        if (!sub) {
            try {
                ...
                await sock.sendMessage(chatId, { text: `✨ *YOUR HUSBAND BOT* ✨\n\n*Usage:* !animu <type>\n*Available Types:* ${apiTypes}\n\n*Powered by RABBI*` }, { quoted: message });
            } catch {
                await sock.sendMessage(chatId, { text: `✨ *YOUR HUSBAND BOT* ✨\n\n*Usage:* !animu <type>\n*Available Types:* ${supported.join(', ')}\n\n*Powered by RABBI*` }, { quoted: message });
            }
            return;
        }
        ...
        await sock.sendMessage(chatId, { text: '❌ *YOUR HUSBAND BOT* ❌\n\nAn error occurred while fetching anime data.' }, { quoted: message });
    }
}
