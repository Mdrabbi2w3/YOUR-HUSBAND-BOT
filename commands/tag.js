...
async function tagCommand(sock, chatId, senderId, messageText, replyMessage, message) {
    ...
    if (replyMessage) {
        ...
        if (Object.keys(messageContent).length > 0) {
            ...
            // Temp file cleanup if any media was downloaded
            if (messageContent.image?.url || messageContent.video?.url || messageContent.document?.url) {
                try { fs.unlinkSync(messageContent.image?.url || messageContent.video?.url || messageContent.document?.url); } catch (e) {}
            }
        }
    } else {
        await sock.sendMessage(chatId, {
